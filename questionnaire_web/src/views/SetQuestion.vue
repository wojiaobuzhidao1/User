    ...
    <el-container>
      ...
      <el-main>
        <el-row class="row-padding" type="flex" justify="space-between" align="middle">
          <el-col :span="5">
            <el-input v-model="title" placeholder="问卷模板名称" @blur="titleInput"></el-input>
          </el-col>
          <el-col :span="5">
  ...
  methods: {
    ...
    init() {
      ...
      if (this.$route.query.questionnaireTemplateId) {
        this.getQuestionnaireTemplateById({
          id: this.$route.query.questionnaireTemplateId
        }).then(y => {
    ...
    validate() {
      ...
      if (this.QuestionnaireTemplate.template.topic.length < 1)
        return this.showWarning("问卷模板至少包含一个问题");
      return true;
    },
    clear() {
      this.clearTemplate();
