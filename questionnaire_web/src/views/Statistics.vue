<template>
  <div>
    ...
    <el-row
      ...
      justify="left"
      align="middle"
      v-for="(item, index) in Questionnaire.list"
      :key="item._id"
    >
      <el-col>
        ...
        <el-row class="questionRow" type="flex" justify="left" align="middle">
          <el-col>
            <el-row
              class="answerRow"
              v-if="item.result[0].type === '单选' || item.result[0].type === '多选'"
            >
              <el-row
                justify="left"
                align="middle"
                class="answerRow"
                v-for="(optionName, index) in item.result[0].optionSort"
                :key="index"
              >
                <el-col :span="5">
            ...
            <el-row class="answerRow" v-if="item.result[0].type === '打分'">
              ...
              <el-row class="additionalRow">
                <el-tag
                  class="additionalTag"
                  type="primary"
                  v-for="(item, index) in item.result"
                  :key="index"
                >{{item.selectContent}}分</el-tag>
              </el-row>
            </el-row>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
        ...
        if (question.result[0].type === "多选") {
          question.result = question.result.reduce((memo, item) => {
            if (item.selectMultipleContent.length === 0) {
              ...
              _.assignIn(item, {
                selectContent: item.selectMultipleContent[0]
              });
              memo.push(item);
            }
            return memo;
    ...
    countResult(questionnaireList) {
      return questionnaireList.map(question => {
        if (
          question.result[0].type === "单选" ||
          question.result[0].type === "多选"
        ) {
          ...
          _.assignIn(question, {
            additionalArr,
            countSelect,
            personSum: question.result.length
          });
          ...
          let answerArr = question.result.reduce((memo, answer) => {
            if (answer.selectContent) {
              memo.push({
                userName: answer.userName,
                additional: answer.selectContent
              });
            }
            return memo;
          }, []);
