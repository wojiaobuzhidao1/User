<template>
  <div>
    <el-row type="flex" justify="end" align="middle" style="padding: 20px">
      <el-col :span="5">
        <el-input v-model="questionnaireName" placeholder="问卷名称"></el-input>
      </el-col>
      ...
      <el-col :span="2" :offset="1">
        <el-button type="primary" size="large" style="postio" @click="add">添加问卷</el-button>
      </el-col>
    </el-row>
    <el-table :data="this.QuestionnaireEach.list" border style="width: 100%;">
      ...
      <el-table-column label="操作">
        <template scope="scope">
          <el-button
            ...
            type="danger"
            size="small"
            @click="changeStatus(scope.row._id, !scope.row.status)"
          >关闭</el-button>
          <el-button
            v-else
            type="primary"
          ...
          >开启</el-button>
          <router-link
            :to="{ path: 'statistics', query: { questionnaireId: scope.row._id, questionnaireName: scope.row.title }}"
          >
...
export default {
  ...
  methods: {
    ...mapActions([
      "getQuestionnaireEachList",
      "getQuestionnaireTemplateList",
      "addQuestionnaire",
      "changeQuestionnaireEachStatus"
    ]),
    init() {
    ...
    add() {
      this.addQuestionnaire({
        ...
        templateId: this.questionnaireTemplateId
      }).then(() => {
        this.getQuestionnaireEachList();
        this.$notify({
          title: "成功",
