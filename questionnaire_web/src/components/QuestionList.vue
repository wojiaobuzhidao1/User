    ...
    <el-row type="flex" justify="left" align="middle" class="markSetRow">
      <el-col :offset="1">
        <el-row
          ...
          v-for="(item,index) in QuestionnaireTemplate.template.topic"
          :key="index"
        >
          <el-col>
            <el-button
              @click="selectedRow(index)"
              :type="selectRow === index ? 'primary' : 'gray'"
              class="questionListText"
            >问题{{index + 1}}({{item.type}})：{{item.question}}</el-button>
          </el-col>
        </el-row>
      </el-col>
    </el-row>
