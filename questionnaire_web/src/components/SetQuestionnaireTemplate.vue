<template>
  <div>
    ...
    <el-table :data="this.QuestionnaireTemplate.list" border style="width: 100%;">
      ...
      <el-table-column label="操作">
        <template scope="scope">
          <router-link
            :to="{ path: 'setQuestion', query: { questionnaireTemplateId: scope.row._id}}"
          >
...
export default {
  ...
  methods: {
    ...
    remove(templateId, index) {
      this.removeTemplateById({ templateId: templateId, index: index }).then(
        error => {
          if (error) {
            this.$notify.error({
              title: "错误",
